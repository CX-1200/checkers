...
void convertInput(PlayerInputMove&, string);
bool legalMove(GameData&, GamePiece&, SaveGamePiece&, char);
void checkKingPromotion(GameData&, GamePiece&);

void removeCaptured(GameData&, GamePiece&);
bool canCaptureFrom(GameData&, GamePiece&, int, int);
...
int main()
{
    ...
    cout << "============================================" << endl;
    cout << setw(28) << "Checkermania" << endl;
    cout << "============================================" << endl;
...
bool loadGame(GameData& data, GamePiece& piece)
{
    ...
    data.superPowerCount = 0;
    for (int row = 0; row < data.boardSize; row++)
        for (int col = 0; col < data.boardSize; col++)
        {
            char powerPiece = piece.powerGrid[row][col];
            if (powerPiece == 'T'|| powerPiece == 'R' || powerPiece == 'L'|| powerPiece == 'D'|| powerPiece == 't'|| powerPiece == 'r'|| powerPiece == 'l'|| powerPiece == 'd')
                data.superPowerCount++;
        }
        ...
        } else if (playerAction == 1)
        {
            ...
            if (validateMove(data, piece))
            {
                ...
                if (piece.pieceCapture)
                    removeCaptured(data, piece);

                // Optional multi-jump: if the same piece can capture again, ask to continue.
                while (piece.pieceCapture && canCaptureFrom(data, piece, piece.moveToRow, piece.moveToCol))
                {
                    int landRow = piece.moveToRow;
                    int landCol = piece.moveToCol;

                    char again;
                    cout << "You can jump again! Continue? (y/n): ";
                    cin >> again;
                    ...
                    PlayerInputMove nextInput;
                    convertInput(nextInput, nextTo);
                    piece.moveFromRow = landRow;
                    piece.moveFromCol = landCol;
                    piece.moveToRow = nextInput.playerMoveRow;
                    piece.moveToCol = nextInput.playerMoveCol;
                    piece.pieceCapture = false;

                    if (validateMove(data, piece) && piece.pieceCapture)
                    {
                        piece.coordinate[piece.moveToRow][piece.moveToCol] = piece.coordinate[landRow][landCol];
                        piece.coordinate[landRow][landCol] = '.';
                        removeCaptured(data, piece);
                    }
                    else
                    {
                        cout << endl << endl;
                        cout << "Not a valid jump. Try again.";
                        cout << endl << endl;
                        piece.moveToRow = landRow;
                        piece.moveToCol = landCol;
...
bool validateMove(GameData& data, GamePiece& piece)
{
    ...
    // Flying King: may move any distance along the diagonal
    if (moveKingPiece)
    {
        int stepRow = (distanceRow > 0) ? 1 : -1;
        int stepCol = (distanceCol > 0) ? 1 : -1;

        int enemyCount = 0;
        int enemyRow = -1, enemyCol = -1;

        int row = piece.moveFromRow + stepRow;
        int col = piece.moveFromCol + stepCol;
        while (row != piece.moveToRow)
        {
            char mid = piece.coordinate[row][col];
            if (mid != '.' && mid != ' ')
            {
                bool isEnemy;
                if (data.currentPlayer == 'x')
        ...
        if (enemyCount == 1)
        {
            piece.middleRow = enemyRow;
            piece.middleCol = enemyCol;
            piece.pieceCapture = true;
            return true;
        }
        return false;
...
void convertInput(PlayerInputMove& playerInput, string playerMove)
{
...
void checkKingPromotion(GameData& data, GamePiece& piece)
{
    ...
    if (promotion)
    {
        ...
        {
            cout << "King promoted! (No superpowers left - only 3 exist.)" << endl;
        }
    }
}

void removeCaptured(GameData& data, GamePiece& piece)
{
...
bool canCaptureFrom(GameData& data, GamePiece& piece, int fromRow, int fromCol)
{
    int direction[4][2] = {{1,1},{1,-1},{-1,1},{-1,-1}};
    int saveFromRow=piece.moveFromRow, saveFromCol=piece.moveFromCol;
    int saveToRow=piece.moveToRow, saveToCol=piece.moveToCol;
    int saveMiddleRow=piece.middleRow, saveMiddleCol=piece.middleCol;
    bool saveCapture=piece.pieceCapture;

    char kingpiece = piece.coordinate[fromRow][fromCol];
    bool isKing = (kingpiece == 'X' || kingpiece == 'O');
    bool found = false;
    piece.moveFromRow = fromRow; piece.moveFromCol = fromCol;

    for (int d = 0; d < 4 && !found; d++)
    {
        int maxDist = isKing ? data.boardSize - 1 : 2;
        for (int dist = 2; dist <= maxDist && !found; dist++)
        {
            piece.moveToRow = fromRow + direction[d][0] * dist;
            piece.moveToCol = fromCol + direction[d][1] * dist;
            piece.pieceCapture = false;
            if (validateMove(data, piece) && piece.pieceCapture) found = true;
        }
    }

    piece.moveFromRow=saveFromRow; 
    piece.moveFromCol=saveFromCol;
    piece.moveToRow=saveToRow;     
    piece.moveToCol=saveToCol;
    piece.middleRow=saveMiddleRow;    
    piece.middleCol=saveMiddleCol;
    piece.pieceCapture=saveCapture;
...
// Superpowers
void chooseSuperpower(GameData&, GamePiece& piece, int row, int col)
{
...
void doTeleport(GameData& data, GamePiece& piece, int kingRow, int kingCol)
{
    ...
    cin >> destination;
    convertInput(inputDestination, destination);

    int destinationRow = inputDestination.playerMoveRow;
    int destinationCol = inputDestination.playerMoveCol;

    if (destinationRow < 0 || destinationRow >= data.boardSize || destinationCol < 0 || destinationCol >= data.boardSize || (destinationRow + destinationCol) % 2 == 0 || piece.coordinate[destinationRow][destinationCol] != '.')
    {
...
void doLight(GameData& data, GamePiece& piece, int kingRow, int kingCol)
{
    ...
    convertInput(toInput, extraMoveDestination);
...
void doDark(GameData& data, GamePiece& piece, int kingRow, int kingCol)
{
    ...
    cin >> captureTarget;
    convertInput(targetInput, captureTarget);
